...
if st.button('Go',on_click=callback) or st.session_state['btn_clicked']:
    ...
    try:
        ...
        model="gpt-4o",
        messages=[
              {"role": "system", "content": """You are a financial analyst. Please analyze the provided market performance data and identify the previous highest value (all-time high) 
              and the previous lowest value (all-time low). Explain these points in simple terms, including the following:
...
Calculate daily, weekly, and monthly volatility and analyse key sources of variation.
